...
namespace LMS.Web.Areas.Identity.Pages.Account
{
    ...
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<ApplicationUser> signInManager, ILogger<LoginModel> logger, IApplicationDbContext context, UserManager<ApplicationUser> userManager)
        {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (Input.Email.IndexOf('@') > -1)
                {
                    ...
                    {
                        user = await _userManager.Users.AsNoTracking().FirstOrDefaultAsync(x => x.UserName == Input.Email);
                        result = await _signInManager.PasswordSignInAsync(Input.Email, Input.Password, Input.RememberMe, lockoutOnFailure: true);
                    }
                    else
                    {
                        user = await _userManager.Users.AsNoTracking().FirstOrDefaultAsync(x => x.Email == Input.Email);
                        if (user == null)
                        {
